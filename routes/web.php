...
Route::prefix('admin')
    ...
    ->group(function (): void {
        ...
        Route::get('/today-deliveries', [TodayDeliveryController::class, 'index'])->name('today-deliveries.index');
        Route::patch('/today-deliveries/addresses/{address}/coordinates', [TodayDeliveryController::class, 'saveCoordinates'])->name('today-deliveries.coordinates');

        Route::get('/custom-orders', [CustomOrderController::class, 'index'])->name('custom-orders.index');
        Route::patch('/custom-orders/{customOrder}/status', [CustomOrderController::class, 'updateStatus'])->name('custom-orders.update-status');
        ...
        Route::post('/daily-deliveries/generate-today', [SubscriptionDeliveryController::class, 'generateToday'])->name('daily-deliveries.generate-today');
        Route::patch('/daily-deliveries/{delivery}/status', [SubscriptionDeliveryController::class, 'updateStatus'])->name('daily-deliveries.update-status');

        /* Event master: reusable event catalog with photo/video/plan/description */
        Route::resource('event-masters', EventMasterController::class)->except(['show']);
        Route::delete('/event-masters/{eventMaster}/media/{media}', [EventMasterController::class, 'destroyMedia'])
            ->name('event-masters.media.destroy');

        Route::get('/event-bookings', [EventBookingController::class, 'index'])->name('event-bookings.index');
        Route::patch('/event-bookings/{eventBooking}/status', [EventBookingController::class, 'updateStatus'])->name('event-bookings.update-status');
...
Route::name('website.')->group(function (): void {
    ...
    Route::get('/delete-account', [AccountDeletionController::class, 'show'])->name('account-delete.form');
    Route::delete('/delete-account', [AccountDeletionController::class, 'destroy'])->middleware('throttle:3,1')->name('account-delete.destroy');
});
